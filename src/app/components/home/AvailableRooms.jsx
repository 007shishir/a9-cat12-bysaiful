...
const AvailableRooms = () => {
  ...
  useEffect(() => {
    const fetchLatestRooms = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/home-rooms', {
          method: 'GET',
          headers: {
  ...
  if (isLoading) {
    return (
      <div className="max-w-7xl mx-auto px-4 w-full mb-12">
        {/* Skeleton Header Loading State */}
        <div className="mb-8 text-center sm:text-left">
          <div className="h-9 bg-base-300 w-64 rounded-xl animate-pulse mb-2 mx-auto sm:mx-0" />
          <div className="h-4 bg-base-200 w-80 rounded-lg animate-pulse mx-auto sm:mx-0" />
        </div>
  ...
  if (rooms.length === 0) {
    return (
      <div className="max-w-7xl mx-auto px-4 w-full mb-12">
        {/* Fallback Header State */}
        <div className="mb-8 text-center sm:text-left">
          <h2 className="text-3xl font-black text-base-content tracking-tight mb-2">Available Rooms</h2>
          <p className="text-sm text-base-content/60 font-medium">Explore the newest workspace setups added by peers across the platform.</p>
        </div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 w-full mb-12">
      ...
      {/* Dynamic Styled Section Headline Block */}
      <div className="mb-8 text-center sm:text-left border-b border-base-content/5 pb-4">
        <h2 className="text-3xl font-black text-base-content tracking-tight mb-2">
          Available Rooms
        </h2>
        <p className="text-sm text-base-content/60 font-medium">
          Instantly lock down high-productivity spaces optimized for private deep focus or collective development tasks.
        </p>
      </div>
