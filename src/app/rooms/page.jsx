"use client";

import React, { useEffect, useState } from 'react';
import Link from 'next/link';
import Image from 'next/image';
import toast from 'react-hot-toast';

const PublicRoomsPage = () => {
  const [rooms, setRooms] = useState([]);
  const [isLoading, setIsLoading] = useState(true);

  // Fetch all listed workspaces across the platform
  useEffect(() => {
    const fetchAllRooms = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/rooms`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json'
          }
        });

        const result = await response.json();

        if (!response.ok) {
          throw new Error(result.message || "Failed to sync platform rooms array data.");
        }

        setRooms(result.data || []);
      } catch (error) {
        toast.error(error.message || "Could not retrieve the rooms catalog.");
      } finally {
        setIsLoading(false);
      }
    };

    fetchAllRooms();
  }, []);

  // Structural Skeleton Loading Loader Grid to match your responsive viewport metrics
  if (isLoading) {
    return (
      <section className="w-full bg-base-100 min-h-screen py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="h-8 bg-base-300 w-48 rounded-xl animate-pulse mb-3" />
          <div className="h-4 bg-base-200 w-80 rounded-lg animate-pulse mb-10" />
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[1, 2, 3].map((n) => (
              <div key={n} className="bg-base-200 border border-base-content/5 h-[400px] rounded-3xl animate-pulse" />
            ))}
          </div>
        </div>
      </section>
    );
  }

  return (
    <section className="w-full bg-base-100 min-h-screen py-12 px-4 sm:px-6 lg:px-8 transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        
        {/* Page Typography Header */}
        <div className="border-b border-base-content/10 pb-6 mb-10 text-center sm:text-left">
          <h1 className="text-3xl font-black text-base-content tracking-tight mb-2">
            Explore Study Rooms
          </h1>
          <p className="text-sm text-base-content/60 font-medium">
            Find and book the perfect quiet workspace tailored to your study session requirements.
          </p>
        </div>

        {/* Empty Catalog View State Handler */}
        {rooms.length === 0 ? (
          <div className="w-full bg-base-200/40 border border-dashed border-base-content/20 rounded-3xl p-12 text-center max-w-xl mx-auto mt-12">
            <div className="w-16 h-16 bg-base-300 rounded-full flex items-center justify-center mx-auto mb-4">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-base-content/40" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
              </svg>
            </div>
            <h3 className="text-xl font-bold text-base-content mb-2">No rooms found</h3>
            <p className="text-sm text-base-content/60 font-medium">
              There are currently no workspace spaces listed right now. Check back later!
            </p>
          </div>
        ) : (
          /* Uniform Flexible Grid Blueprint: 3 cols desktop, 2 cols tablet, 1 col mobile */
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {rooms.map((room) => {
              const maxChips = 3;
              const hasMoreChips = room.amenities?.length > maxChips;
              const runningChips = room.amenities?.slice(0, maxChips) || [];

              return (
                <div 
                  key={room._id} 
                  className="card card-compact bg-base-100 border border-base-content/10 shadow-md hover:shadow-xl rounded-3xl overflow-hidden transition-all duration-300 flex flex-col h-full"
                >
                  
                  {/* Uniform Aspect-Ratio Image Mask Section Container */}
                  <div className="relative w-full aspect-[16/10] bg-base-300 overflow-hidden">
                    <Image 
                      src={room.image || "https://images.unsplash.com/photo-1497366216548-37526070297c?auto=format&fit=crop&w=600&q=80"}
                      alt={room.name}
                      fill
                      sizes="(max-w-768px) 100vw, (max-w-1200px) 50vw, 33vw"
                      className="object-cover transition-transform duration-500 hover:scale-105"
                      priority
                    />
                    {/* Location Badge Overlay */}
                    <div className="absolute top-4 right-4 badge badge-neutral font-bold rounded-lg px-3 py-2 text-xs shadow-sm">
                      {room.floor}
                    </div>
                  </div>

                  {/* Card Content Area Layout */}
                  <div className="card-body p-5 flex flex-col flex-grow justify-between">
                    
                    <div>
                      {/* Room Heading Title */}
                      <h2 className="card-title font-black text-xl text-base-content tracking-tight line-clamp-1 mb-1">
                        {room.name}
                      </h2>

                      {/* Seat Capacity Meta Data Subtitle */}
                      <div className="flex items-center gap-2 mb-3 text-xs text-base-content/50 font-bold uppercase tracking-wider">
                        <span>Capacity: {room.capacity} {room.capacity === 1 ? 'person' : 'people'}</span>
                      </div>

                      {/* Truncated Description Content Vector */}
                      <p className="text-sm font-medium text-base-content/70 leading-relaxed mb-4 line-clamp-2">
                        {room.description}
                      </p>
                    </div>

                    <div>
                      {/* Amenities Horizontal Chip Stream Layer */}
                      <div className="flex flex-wrap gap-1.5 mb-5 min-h-[26px]">
                        {runningChips.map((amenity, idx) => (
                          <span key={idx} className="badge badge-sm bg-base-200 border-none text-base-content/80 font-semibold rounded-md px-2 py-2.5">
                            {amenity}
                          </span>
                        ))}
                        {hasMoreChips && (
                          <span className="badge badge-sm badge-primary badge-outline font-bold rounded-md px-2 py-2.5">
                            +{room.amenities.length - maxChips} more
                          </span>
                        )}
                      </div>

                      {/* Action Interface Row Controls Block */}
                      <div className="flex items-center justify-between border-t border-base-content/5 pt-4 mt-auto">
                        <div className="flex flex-col">
                          <span className="text-xs text-base-content/40 font-bold uppercase tracking-wider">Hourly Cost</span>
                          <span className="text-xl font-black text-primary">${room.hourlyRate}<span className="text-xs font-medium text-base-content/60">/hr</span></span>
                        </div>
                        <Link 
                          href={`/rooms/${room._id}`}
                          className="btn btn-primary btn-sm rounded-xl font-bold normal-case px-5 shadow-sm"
                        >
                          View Details
                        </Link>
                      </div>
                    </div>

                  </div>

                </div>
              );
            })}
          </div>
        )}

      </div>
    </section>
  );
};

export default PublicRoomsPage;